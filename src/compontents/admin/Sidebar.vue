	...
	>
		<SimpleBar class="h-full flex flex-col overflow-y-auto">
			<div class="p-3 pb-2 flex justify-between items-center">
				...
				<button class="shrink-0 transition-colors hover:bg-gray-200 rounded text-gray-600 px-1 lg:hidden" @click.prevent="emits('toggleSidebar')">
					<svg class="w-8 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="black">
			...
			<ul class="flex-1 flex flex-col justify-between p-3">
				<div>
					<template v-for="(item, index) in links" :key="index">
						...
						<li v-else-if="item?.link && item?.label" class="my-1">
							<a
								:href="item.link"
								class="relative flex items-center gap-2 w-full py-1 px-2 rounded cursor-pointer transition-colors hover:bg-blue-50 hover:text-blue-800 text-gray-600 group"
				...
				<div>
					<li class="my-1">
						<a
							href="#"
							class="relative flex gap-2 items-center w-full py-1 px-2 rounded cursor-pointer transition-colors hover:bg-blue-50 hover:text-blue-800 text-gray-600 group"
						>
							<svg class="w-5 h-5 shrink-0 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
								<path d="M0 0h24v24H0V0z" fill="none" />
								<path
							...
							<div class="overflow-hidden whitespace-nowrap text-ellipsis">
								<span class="text-sm text-current">Settings</span>
							</div>
						</a>
					</li>
				</div>
